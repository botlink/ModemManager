...
static void set_reg_msg_handlers (Dial3gppContext *ctx, gboolean enable)
{
    ...
    if (enable)
        mm_obj_dbg (ctx->self, "Enabling unsolicited registration message handlers.");
    else
        mm_obj_dbg (ctx->self, "Disabling unsolicited registration message handlers.");
}
...
dial_3gpp_context_step (GTask *task)
{
    ...
    mm_obj_dbg (ctx->self, "Dial3gpp Step : %d", ctx->step);
               ...
               GTask *task)
{
    ...
    /* Parse response */
    if (!mm_3gpp_parse_cgpaddr_write_response (response, ctx->self, &cid, &ipv4addr, &ipv6addr)) {
...
get_ip_config_3gpp_context_step (GTask *task)
{
    ...
    mm_obj_dbg (ctx->self, "GetIPConfig Step : %d", ctx->step);
                        ...
                        GTask        *task)
{
    ...
    if (error) {
        ...
        {
            mm_obj_dbg (ctx->self, "PDP context deactivation failed (not fatal): %s", error->message);
        }
        g_error_free (error);
    }
...
disconnect_3gpp_context_step (GTask *task)
{
    ...
    mm_obj_dbg (ctx->self, "Disconnect3gpp Step : %d", ctx->step);
